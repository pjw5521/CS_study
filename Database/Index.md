...
- 데이터베이스의 응용, 데이터베이스 자체, 운영체제의 조정을 통해 최적의 자원으로 최적의 성능을 얻을 수 있도록 개선하는 작업이다. 

![image](https://user-images.githubusercontent.com/60414900/164644027-0514001b-733b-4771-8e75-53297a545702.png)
...
_인덱싱은 바로 SQL 튜닝 중 하나이다._

ref) http://blog.skby.net/데이터베이스-튜닝-db-tuning/

</br>

### 1. 인덱스(Index) 정의

_추가적인 쓰기 작업과 저장 공간을 활용하여 데이터베이스 테이블의 검색 속도를 향상시키기 위한 자료구조이다_  

책의 목차,색인과 같다고 생각하면된다. 데이터와 데이터의 위치를 포함한 자료구조를 생성하여 빠르게 조회 할 수록 돕고 있다. 
일반적으로 우리가 사용하는 DBMS는 모든 데이터를 검색하는 Full-Scan을 기반으로 데이터를 조회하는데 데이터가 몇백만건 이상이라면 시간이 오래 걸린다. 
그래서 칼럼의 값과 해당 레코드가 저장된 주소를 키와 값의 쌍으로 인덱스를 생성한다. 
...
1. 읽기의 성능 효율을 증가시키는 한편 수정, 삭제, 삽입에 대한 연산 효율에 방해될 수 있다.

Why?
DBMS의 인덱스는 항상 정렬된 상태를 유지하기 때문에 Select 와 같은 읽기 속도는 빠르지만 새로운 값을 추가하거나 삭제, 수정하는 경우에는 쿼리문의 효율을 감소 시킨다. 즉 데이터의 저장 성능을 희생하여 읽기의 최적화 시키는 것이다. 따라서 인덱스를 활용할 때는 어떤 칼럼을 인덱스로 활용할지 심사숙고가 필요하다. 

2. 인덱스를 관리하기 위한 저장공간이 필요하기 때문에 오버헤드가 발생할 수 있다.
...
#### 3-3 Primary Index vs Secondary Index
